...
public class Connect4
{
   ...
   public static void main(String[] args) 
   {
      ...
      do
      {
         ...
         catch (NumberFormatException e)   // if there is an issue reading the file, it's because the user is not a returning user
         {
            System.out.println("\nFile Not Found.\nYou are not a returning user.\nPlease try again.\n");
            valid = false;
         } 
      ...
      do
      {
         ...
         do
         {
            ...
            // Computer move
               ...
               do
               {
                  computerMove = calculate(board, round, playerPiece, computerPiece); // pick a random number between 0 and 6
   ...
   */
   ...
   public static int calculate (char[][] board, int round, char playerPiece, char computerPiece)
   {
      int move = 0;
      ...
      // try to block connect 4s or get connect 4s (winning given priority)
               ...
               if (board[i][j] == board[i][j+1] && board[i][j] == board[i][j+2] && (board[i][j] == 'X' || board[i][j] == 'O'))
               {
                  if (j > 0)
                  {
                     ...
                     if (board[i][j-1] == ' ' && moveFound == false)
                     {
                        ...
                        {
                           if (board[i][j-1] == computerPiece)
                           {
                              move = (j-1);
                              moveFound = true;
                           }
                           else if (board[i][j-1] == playerPiece && moveFound == false)
                           {
                              move = (j-1);
                           }
                        }
                        ...
                        {
                           if (board[i+1][j-1] == 'X' || board[i+1][j-1] == 'O' && moveFound == false) // if it's not on the first row, make sure that there is a piece under
                           {
                              if (board[i][j-1] == computerPiece)
                              {
                                 move = (j-1);
                           ...
                           else if (board[i+1][j-1] == 'X' || board[i+1][j-1] == 'O' && moveFound == false) // if it's not on the first row, make sure that there is a piece under
                           {
                              if (board[i][j-1] == computerPiece)
                              {
                                 move = (j-1);
                  ...
                  if (j < (COLUMNS-3) && moveFound == false)
                  {
                     ...
                     if (board[i][j+3] == ' ' && moveFound == false)
                     {
                        if (i == (ROWS-1))   // if on first row, you can place it
                        {
                           if (board[i][j+3] == computerPiece)
                           {
                              move = (j+3);
                              moveFound = true;
                           }
                           else if (board[i][j+3] == playerPiece && moveFound == false)
                           {
                              move = (j+3);
                           }
                        }
                        ...
                        {
                           if (board[i+1][j+3] == 'X' || board[i+1][j+3] == 'O' && moveFound == false)// if it's not on the first row, make sure that there is a piece under
                           {
                              if (board[i][j+3] == computerPiece)
                              {
                                 move = (j+3);
            ...
            for (int j = 0; j < COLUMNS; j++)
            {
               if (board[i][j] == board[i+1][j] && board[i][j] == board[i+2][j] && (board[i][j] == 'X' || board[i][j] == 'O'))
               {
                  if (i > 0)
                  {
                     ...
                     {
                        if (board[i-1][j] == computerPiece)
                        {
                           move = j;
                           moveFound = true;
                        }
                        else if (board[i-1][j] == playerPiece && moveFound == false)
                        {
                           move = j;
                        }
                     }
            ...
            for (int j = 0; j <= (COLUMNS-3); j++)
            {
               if (board[i][j] == board[i-1][j+1] && board[i][j] == board[i-2][j+2] && (board[i][j] == 'X' || board[i][j] == 'O'))
               {
                  if (j > 0 && i < (ROWS-1)) 
                  {
                     ...
                     {
                        if (i == (ROWS-2) && moveFound == false)   // if on first row, you can place it
                        {
                           if (board[i+1][j-1] == computerPiece)
                           {
                              move = (j-1);
                              moveFound = true;
                           }
                           else if (board[i+1][j-1] == playerPiece && moveFound == false)
                           {
                              move = (j-1);
                           }
                        }
                        ...
                        {
                           if (board[i+2][j-1] == 'X' || board[i+2][j-1] == 'O' && moveFound == false) // if it's not on the first row, make sure that there is a piece under
                           {
                              if (board[i+1][j-1] == computerPiece)
                              {
                                 move = (j-1);
                  ...
                  if (j < (COLUMNS-3) && i > (ROWS-4) && moveFound == false)
                  {
                     ...
                     {
                        if (board[i-2][j+3] == 'X' || board[i-2][j+3] == 'O' && moveFound == false) // if it's not on the first row, make sure that there is a piece under
                        {
                           if (board[i-3][j+3] == computerPiece)
                           {
                              move = (j+3);
                              moveFound = true;
                           }
                           else if (board[i-3][j+3] == playerPiece && moveFound == false)
                           {
                              move = (j+3);
                           }
                        }
