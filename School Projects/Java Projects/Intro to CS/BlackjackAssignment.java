...
public class BlackjackAssignment
{
   public static void main(String[] args) 
   {
      ...
      int arrayLength = 0; // variable to help deal with tracking the cards picked
      int[] cardsInHand = new int[arrayLength]; // next 4 are used to keep track of cards picked
      int[] suitsInHand = new int[arrayLength];
      int[] cardsComputer = new int[arrayLength];
      int[] suitsComputer = new int[arrayLength];
      int card, aceChoice, suit, pickUser, pickComp;  // store random card, choice of ace value, random suit, picks from user and picks from computer respectively
      ...
      do
      {
         ...
         computerScore = 0;
         pickUser = -1; // reset to keep track of picks
         arrayLength = 0;
         cardsInHand = reset(cardsInHand);  // reset using method
         suitsInHand = reset(suitsInHand);
         ...
         System.out.printf("You currently have $%.2f.%n", bankAccount);
         ...
         do 
         {
            ...
            cardsInHand = addingAnElement(cardsInHand); // add an element to arrays using method
            suitsInHand = addingAnElement(suitsInHand);
            cardsInHand[pickUser] = card; // store all cards and suits 
            suitsInHand[pickUser] = suit;
            arrayLength = cardsInHand.length;   // for loop; size of array
            ...
            // what cards the user has
            System.out.println("The cards in your hand include: ");
            
            for (int i = 0; i < arrayLength; i++)  // loop through all picks
            {
         ...
         pickComp = -1; // reset 
         arrayLength = 0;
         cardsComputer = reset(cardsComputer);
         suitsComputer = reset(suitsComputer);
         ...
         if (playerScore < WINNING_SCORE)
         {
            ...
            while (computerScore <= 16)   // while loop will continue as long as the computer's score is 16 or less
            {
               ...
               cardsComputer = addingAnElement(cardsComputer);   // add element to arrays using method
               suitsComputer = addingAnElement(suitsComputer);
               cardsComputer[pickComp] = card;  // stores cards and suits of all picks
               suitsComputer[pickComp] = suit;
               arrayLength = cardsComputer.length; // maximum length of array; used for loop
               ...
               // says what cards the computer has picked in the round
            ...
            // recount number of picks and what cards and suits the user picked
            ...
            // recount number of picks and what cards and suits the computer picked
   ...
   public static int[] addingAnElement (int[] cardsIncrease) // adding an element to the card array for the user
   {
      int newLength = cardsIncrease.length + 1;
      int[] biggerArray = new int [newLength];
      for (int i = 0; i < newLength-1; i++) 
      {
         biggerArray[i] = cardsIncrease[i];
      }
      return biggerArray;
   }
   
   public static int[] reset (int[] cardsReset)  // reset the card array for the user as a new round will start
   {
      int[] resetArray = new int [0];
